...
*/
                ...
                // total score for all error modes
...
*/
    ...
    void calculate_weights(
      ...
      for (int sample_index = start_sample_index; sample_index < samples; sample_index += blockDim.x * gridDim.x) {
        auto weight = expf(-u_weights[sample_index] / c_lambda );
        if( isnan(weight) ){
            weight = 0.0;
        }
    ...
    void calculate_mppi_control(
      ...
      for (int sample_index = start_sample_index; sample_index < samples; sample_index += blockDim.x * gridDim.x) {
        float weight = 0.0;
        if( !isnan( u_weight_total[0] ) && u_weight_total[0] ) {
        ...
        for (int control_index = start_control_index; control_index < num_controls; control_index += blockDim.y * gridDim.y) {
            ...
            if( !isEqual( weight, 0 ) ) {
                ...
                // u_dist[dist_index].a *= weight;
                // u_dist[dist_index].delta *= weight;
            } else {
    ...
    def find_control(self, costmap, origin, resolution, x_init, x_goal, x_nom, u_nom, actors, dt):
        ...
        self.optimization_args["x_goal"] = x_goal
        ...
        cuda.memcpy_dtoh(u_weights, u_weight_gpu)
        ...
        return u_mppi, u_dist.reshape((self.samples, num_control_elements, -1)), u_weights
