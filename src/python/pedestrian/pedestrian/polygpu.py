...
# BUGBUG - pycuda autoinit does not play nice with other cuda instances
#          -- see: https://github.com/lebedov/scikit-cuda/issues/330,
#                  https://stackoverflow.com/questions/50125944/could-pycuda-and-tensorflow-work-together
#
...
mod = SourceModule(
    ...
    __device__
    bool point_in_polygon(const float *polygon_data, const int num_vertices, float px, float py ) {
    ...
    void check_points(const float *polygon_data, int num_vertices,
        ...
        for (int i = start_index; i < num_points; i += stride) {
            ...
            result_data[i] = point_in_polygon( polygon_data, num_vertices, px, py );
        }
    }
    ...
    __device__ float
    line_range( const float *polygon_list, const int * polygon_indices, int num_polygons, float sx, float sy, float angle, float max_range, float resolution, int *polygon_index ) {

        float ex, ey;
        auto x_inc = cos(angle) * resolution * 0.5;
        auto y_inc = sin(angle) * resolution * 0.5;
        auto dist = 0.0;
        ...
        while( dist < max_range ) {
            ...
            for( int i = 0; i < num_polygons; i++ ) {
                if( point_in_polygon( &polygon_list[ polygon_indices[i] * 2], size_t(polygon_indices[i+1] - polygon_indices[i]), ex, ey ) ) {
    ...
    faux_ray(const float *polygon_list, const int* polygon_indices, int num_polygons, float start_x, float start_y,
        ...
        for (auto i = start_index; i < num_rays; i += stride) {
            ...
            results[i] = line_range(polygon_list, polygon_indices, num_polygons, start_x, start_y, angle, max_range, resolution, &indices[i] );
        }
    }
"""
)
...
def faux_scan(polygons, origin, angle_start, angle_inc, num_rays, max_range, resolution):
    ...
    if not len(polygons):
        return np.ones((num_rays,), dtype=np.float32) * -1.0, np.ones((num_rays,), dtype=np.int32) * 0x7FFFFFFF
    ...
    distances_gpu = cuda.mem_alloc(scan_size)
    cuda.memset_d32(distances_gpu, 0, num_rays)

    index_size = num_rays * np.zeros((1,), dtype=np.int32).nbytes
    indices_gpu = cuda.mem_alloc(index_size)
    cuda.memset_d32(indices_gpu, 0x7FFFFFFF, num_rays)
    ...
    indices = np.zeros(num_rays, dtype=np.int32)
