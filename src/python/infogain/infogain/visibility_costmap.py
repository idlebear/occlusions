...
def build_visibility_costmap(obs, map, obs_trajectory, target_trajectory, v_des, dt):
    ...
    for index in range(start_index + 1, len(target_trajectory.x)):
        ...
        _x = int(
            (target_trajectory.x[index] - target_trajectory.x[0]) / GRID_RESOLUTION
            + GRID_SIZE
        )
        _y = int(
            (target_trajectory.y[index] - target_trajectory.y[0]) / GRID_RESOLUTION
            + GRID_SIZE
        )
    ...
    toc = time()
    print(f"Time to generate target points:{toc-tic:0.5}")
    ...
    # for information gain -- not necessary if we're just evaluating
    # visibility directly
    #
    ...
    toctoc = time()
    print(
        f"Time to generate grid: {toctoc-toc:0.5} -- Total visibility time: {toctoc - tic:0.5}"
    )
