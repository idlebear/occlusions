...
def update_visibility_costmap(
    costmap, obs, map, origin, resolution, obs_trajectory, target_trajectory, v_des, dt
):
    ...
    obs_pts = [
        ...
        for y, x in zip(*obs_pts)
        if y > -LANE_WIDTH + GRID_SIZE // 2
    ]
    ...
    for index in range(start_index + 1, len(target_trajectory.x)):
        ...
        dy = max(1, np.ceil(t * MAX_PEDESTRIAN_SPEED * 1.5 / resolution))
        ...
        target_pts.extend(
            [
                ...
                or (
                    not out_of_obs
                    and obs[int(y - GRID_SIZE / 2), int(_x - GRID_SIZE / 2)]
                    > OCCUPANCY_THRESHOLD
