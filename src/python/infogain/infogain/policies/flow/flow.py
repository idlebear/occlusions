...
def flow(probability_grid, velocity_grid, scale=1, timesteps=1, dt=0.1, mode='nearest'):
    ...
    for t in range(timesteps):
        ...
        mask = (np.sum(velocity_grid, axis=2) != 0).astype(float)
        probability_grid = probability_grid * mask
        xx *= mask.astype(int)
        yy *= mask.astype(int)
        ...
        k_prob_11[y1, x1] = k_prob_11[y1, x1] + probability_grid * p11
        k_prob_12[y1, x2] = k_prob_12[y1, x2] + probability_grid * p12
