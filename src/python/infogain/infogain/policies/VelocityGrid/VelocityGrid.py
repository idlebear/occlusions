...
class VelocityGrid:
    def __init__(
        ...
        if self.debug:
            ...
            for i in range(num_plots):
                self.maps.append(
                    self.map_ax[i].imshow(
                        np.zeros([self.grid_rows, self.grid_cols, 3], dtype=np.uint8)
                    )
                )
    ...
    def reset(self):
        ...
        try:
            self.probabilityMap = (
                np.ones([self.grid_rows, self.grid_cols]).astype(np.float32) * self.l0
            )
            self.velocityMap = np.zeros([self.grid_rows, self.grid_cols, 2]).astype(
                np.float32
            )

            # set up a grid of points in a convenient form for checking visibility
            xx, yy = np.meshgrid(
                np.arange(self.grid_cols), np.arange(self.grid_rows), indexing="xy"
            )
            xx = xx * self.resolution + 0.5 * self.resolution
            yy = yy * self.resolution + 0.5 * self.resolution
            self.grid_pts = np.array(
    ...
    def move_origin(self, new_origin):
        ...
        if old_x_max - old_x_min > 0 and old_y_max - old_y_min > 0:
            ...
            tmp_prob_grid[
                new_y_min:new_y_max, new_x_min:new_x_max
            ] = self.probabilityMap[old_y_min:old_y_max, old_x_min:old_x_max]
            self.probabilityMap = tmp_prob_grid

            tmp_vel_grid[
                new_y_min:new_y_max, new_x_min:new_x_max, :
            ] = self.velocityMap[old_y_min:old_y_max, old_x_min:old_x_max, :]
        ...
        try:
            ...
            if visibility is not None:
                ...
                for agent in agents:
                    ...
                    for x in np.arange(
                        corrected_pos[0] - dx / 2,
                        corrected_pos[0] + dx / 2,
                        self.resolution,
                    ):
                        for y in np.arange(
                            corrected_pos[1] - dy / 2,
                            corrected_pos[1] + dy / 2,
                            self.resolution,
                        ):
                ...
                self.probabilityMap = np.clip(
                    self.probabilityMap, MIN_PROBABILITY, MAX_PROBABILITY
                )

            if self.debug:
                # draw the probability and velocity grid
                map_img = Image.fromarray(
                    np.flipud(((1 - self.__probabilityMap()) * 255.0).astype(np.uint8))
                ).convert("RGB")
                self.maps[0].set_data(map_img)

                map_img = (
                    Image.fromarray(
                        np.flipud((self.velocityMap[:, :, 0] / MAX_CAR_SPEED) * 255.0)
                    )
                    .astype(np.uint8)
                    .convert("RGB")
                )
                ...
                map_img = (
                    Image.fromarray(
                        np.flipud((self.velocityMap[:, :, 1] / MAX_CAR_SPEED) * 255.0)
                    )
                    .astype(np.uint8)
                    .convert("RGB")
                )
